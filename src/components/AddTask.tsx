...
const AddTask = observer(() => {
  ...
  const [assignee, setAssignee] = useState<string>(""); // New state for assignee
  const [priority, setPriority] = useState<string>(""); // New state for priority
  ...
  const handleNewTask = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } else if (!assignee) { // Validate assignee
      setError("Please select an assignee for the task");
    } else if (!priority) { // Validate priority
      ...
      <DialogContent className="sm:max-w-2xl">
        ...
        <form onSubmit={handleNewTask}>
          <div className="grid gap-4 py-4">
            {/* Existing code for title, description, and status */}
