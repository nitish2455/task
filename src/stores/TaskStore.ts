...
export const TaskModel = types.model("Task", {
  ...
  status: types.string,
  assignee: types.optional(types.maybeNull(types.string), ""), 
  priority: types.optional(types.maybeNull(types.string), ""), 
});
...
export const TaskStore = types
  ...
  .actions((self) => {
    return {
      ...
      editTask(
        id: string,
        editedTask: { title: string; description: string; status: string; assignee: string; priority: string}
      ) {
